...
void dcMotorTest(void)
{
  ...
  // Forward 3V for 5s
  DCMotor_SetVoltage(&spool_motor, 8.0f);
  HAL_Delay(5000);
    
  // Reverse 2V for 5s
  DCMotor_SetVoltage(&spool_motor, -6.0f);
  HAL_Delay(5000);
    
  // Coast (free-wheel) for 10s
...
void AS5600_Test(void) {
    ...
    // status = HAL_I2C_IsDeviceReady(&hi2c1, AS5600_I2C_ADDR, 3, 1000);
//    if (status != HAL_OK) {
