...
alpha = deg2rad(45);  % V-groove half-angle
d_offset = R_p / sin(alpha);  % ~2.12 mm

% Pressure angle
pressureAngle1 = atan(cycloid1.v/omega/R_p); % [rad]
pressureAngle2 = atan(cycloid2.v/omega/R_p); % [rad]
pressureAngle = [pressureAngle1 pressureAngle2];

% normal vector in theta-z plane
pitchNormal = [-sin(pressureAngle) cos(pressureAngle)];

% map pitch curve to surface
R_groove1 = R_p - R_f*sin(pressureAngle1);
z_groove1 = cycloid1.s + R_f*cos(pressureAngle1);
R_groove2 = R_p - R_f*sin(pressureAngle2);
z_groove2 = cycloid2.s + R_f*cos(pressureAngle2);

% radCurvature1 = -((1+(cycloid1.v/omega/R_p).^2).^(3/2)) ./ ...
%                     (cycloid1.a./(omega^2*R_p^2));

% vertices for 90 deg V-groove
vertex_z1 = cycloid1.s + d_offset*cos(pressureAngle1);
vertex_z2 = cycloid2.s + d_offset*cos(pressureAngle2);
vertex_r1 = R_p - d_offset*sin(pressureAngle1);
vertex_r2 = R_p - d_offset*sin(pressureAngle2);

% % follower contact point in cylindrical coords
% % rise
% z_s1 = cycloid1.s + R_f*cos(pressureAngle1);
...
% generate spline from pitch curve
spline1.x = R_groove1.*cos(camAngles1);
spline1.y = R_groove1.*sin(camAngles1);
spline1.z = z_groove1;

spline2.x = R_groove2.*cos(camAngles2);
spline2.y = R_groove2.*sin(camAngles2);
spline2.z  = z_groove2;
...
spline1.x = spline1.x.*m2cm;
spline1.y = spline1.y.*m2cm;
spline1.z = spline1.z.*m2cm;

spline2.x = spline2.x.*m2cm;
spline2.y = spline2.y.*m2cm;
spline2.z = spline2.z.*m2cm;

% concantenate into single spline
...
writematrix([spline.x', spline.y', spline.z'], 'cam_pitch_full.csv');
